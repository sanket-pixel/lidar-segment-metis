...
class PixelShuffleConv(nn.Module):
    """
    Replaces nn.PixelShuffle(2) with a ConvTranspose2d using fixed 0/1 weights.
    Bypasses ONNX DepthToSpace / 6D Reshape issues on Axelera Voyager SDK.
    """
    def __init__(self, in_channels, upscale_factor=2):
        ...
        # Construct fixed mapping matrix
        W = torch.zeros(in_channels, self.out_channels, upscale_factor, upscale_factor)
        for c in range(self.out_channels):
            for i in range(upscale_factor):
                for j in range(upscale_factor):
                    in_idx = c * (upscale_factor ** 2) + i * upscale_factor + j
                    W[in_idx, c, i, j] = 1.0
...
class UpBlock(nn.Module):
    def __init__(self, in_filters, out_filters, dropout_rate, drop_out=True):
        ...
        # Custom ConvTranspose-based PixelShuffle to satisfy Voyager compiler
        self.pixel_shuffle = PixelShuffleConv(in_channels=in_filters, upscale_factor=2)
...
class SalsaNext(nn.Module):
    def __init__(self, nclasses=20, return_softmax=False):
